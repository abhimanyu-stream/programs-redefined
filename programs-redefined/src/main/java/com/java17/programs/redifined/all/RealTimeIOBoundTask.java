...
public class RealTimeIOBoundTask {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        for (int i = 1; i <= numberOfTasks; i++) {
            ...
            FutureTask<String> task = new FutureTask<>(() -> {
                ...
                // Simulate I/O-bound: mock HTTP call or file read
                System.out.println("[" + threadName + "] Starting Task " + taskId);
                Thread.sleep(500 + (int)(Math.random() * 1500)); // simulate latency

                // Pretend to process some data
                String resultData = "Data from job-" + taskId;
                return "Processed: " + resultData + " by " + threadName;
            });

            futures.add(task);
            boolean result = taskQueue.offer(task);
            System.out.println("Task " + taskId + " submitted: " + result);
        }
