...
public class RealTimeCPUboundTask {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        for (int i = 1; i <= numberOfTasks; i++) {
            ...
            FutureTask<String> task = new FutureTask<>(() -> {
                ...
                // CPU-bound: calculate nth prime number
                int n = 10000 + taskId; // size of work varies per task
                int prime = calculateNthPrime(n);

                return "Task " + taskId + " → " + n + "th prime = " + prime + " processed by " + threadName;
            });

            futures.add(task);
            boolean result = taskQueue.offer(task);
            System.out.println("Submitted Task " + taskId + ": " + result);
        }
